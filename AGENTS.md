...
- `sync-main.sh` is ignored local tooling for GitHub-to-Forgejo sync; it should replay GitHub-only commits onto local `main` with `git cherry-pick -S` and push `main:main` to Forgejo, never direct-push `github/main:main`, because direct ref pushes preserve GitHub-created commit objects and signatures.
