...
- In Codex/CI-like environments, Turbopack may fail `next/font/google` fetches with TLS errors; older Next 16.1 builds used `experimental.turbopackUseSystemTlsCerts = true`, but Next 16.2 rejects that key, so do not re-add it without checking current Next config support.
...
- Unified trip persistence should serialize writes per file and commit via temp-file rename (atomic replace); overlapping saves to the same `trip-*.json` can otherwise interleave and produce malformed JSON with NUL padding/trailing stale bytes.
- `loadUnifiedTripData` now attempts one-pass self-healing for malformed `trip-*.json` files by parsing the valid JSON prefix (parser position / first NUL cutoff), backing up the original bytes under `data/backups/corrupted-trip-*.json.corrupt`, and atomically rewriting the recovered trip file.
...
- Service worker offline navigation fallback must be path-aware (`/admin*` should prefer cached admin shell; `/maps|/map|/embed|/calendars*` should prefer public shell) to avoid cross-serving the public UI when admin navigation falls back under stale/offline conditions.
- Service worker fetch handling must short-circuit cross-origin requests (except explicit allowed hosts like OSM tiles); otherwise `navigate` interception can hijack outbound links to sibling `*.bdamokos.org` subdomains from controlled pages.
...
- Service worker install should only pre-cache a minimal shell and must not block activation on broad route/data pre-cache fetches; long-running install pre-cache can leave registrations stuck in `installing` and keep stale workers controlling chunk/navigation behavior.
- Service worker-driven offline warmup for route documents should mark client fetches with `x-travel-tracker-precache: 1`; plain `fetch('/admin/...')` is not a navigation request and otherwise won't enter the app-shell `networkFirst` branch.
- Admin offline-ready UX now listens for `travel-tracker-service-worker-offline-ready` and can read latest warmup detail from `travel-tracker-service-worker-offline-ready-detail-v1` in `sessionStorage`.
- Instagram importer hardening now lives in `src/app/lib/instagramImportUtils.ts`: API routes should normalize `@handle`/URL usernames, default to IG app id `936619743392459` when unset, optionally send session cookies from `INSTAGRAM_SESSIONID`/`INSTAGRAM_CSRFTOKEN`/`INSTAGRAM_DS_USER_ID`, and parse/dedupe posts from varied payload shapes before failing.
- `normalizeInstagramUsername` must validate parsed hosts against an explicit Instagram hostname allowlist (`instagram.com`, `www.instagram.com`, `i.instagram.com`) instead of substring checks like `.includes('instagram.com')`, which accept lookalike hosts such as `evil-instagram.com`.
- Service worker stale-while-revalidate handlers must register background revalidation with `FetchEvent.waitUntil`; non-GET data cache invalidation should run only after successful mutation responses and in background, while navigation cache lookups should ignore search params and prefer cost/travel-specific admin fallbacks to avoid redirecting edits to `/admin`.
- Admin app-shell fallback matching should preserve exact query params for tab-specific routes (for example `/admin?tab=cost`), only using `ignoreSearch` for path-only fallback keys; `/api/cost-tracking*` mutations must invalidate related `/api/travel-data*` and `/admin/api/accommodations*` cache entries because some cost endpoints persist travel-side changes.
- `OFFLINE_TILE_PNG_BYTES` in `public/sw.js` must stay a valid 1x1 transparent RGBA PNG (IHDR color type 6 with IDAT decompressing to `[0,0,0,0,0]`); app-shell pre-cache result handling should log all failed URLs before throwing on critical failures.
- Service worker runtime caching must never persist redirected responses (`Response.redirected === true`) to avoid replaying redirected HTML under non-redirect route keys, and client reload on `controllerchange` should only happen when this tab explicitly initiated `SKIP_WAITING` for an update.
- `ServiceWorkerRegistration` now performs a one-time legacy SW reset for cache-version migrations (unregister existing workers + delete old `app-shell/static/data/tiles` caches before re-registering) and warmup retry counts should only include transient failures (network/5xx/429), not permanent 4xx misses.
- Avoid hardcoding cache version twice: `ServiceWorkerRegistration` should derive `CACHE_VERSION` by reading `/sw.js` and use that value for legacy-reset key/cache detection; `preCacheAppShell` should aggregate all critical failures and throw one combined error containing every failed critical URL reason.
- `initializeServiceWorker` should guard async continuation with `isUnmounted` after awaited steps (cache-version discovery and legacy reset) and catch reset/registration exceptions so teardown cannot race into post-unmount service-worker setup.
- Daily trip-spending bars in `CostSummaryDashboard` are now interactive buttons: selecting a day opens an `AccessibleModal` with same-day actual trip expenses (date-matched via local-day normalization), per-item details, and day total.
- Trip editor location cards should avoid duplicate expense summaries: keep `LinkedExpensesDisplay` as the single green source of linked expenses and show totals there (allocated/split-aware, grouped by currency).
- Browser-side route generation should call a same-origin OSRM proxy (e.g. `/api/routing/osrm`) instead of `router.project-osrm.org` directly to avoid cross-origin failures/no-CORS 5xx masking in admin route editors; `ServiceWorkerRegistration` should catch/ignore transient `registration.update()` `InvalidStateError` rejections (notably Safari/WebKit `"newestWorker is null"`) so they do not surface as unhandled promise rejections.
- `AccessibleDatePicker` should close its popover immediately after a successful calendar date selection; keep that behavior enforced in the shared component rather than relying on individual form wrappers.
- `AccessibleDatePicker` calendar popup should support typed day selection in the visible month (for example `2` then `0` selects day 20), buffering 1-2 digit input in the shared calendar component rather than per-form handlers.
- Admin cost-tracker list loading can request `/api/cost-tracking/list?includeCostData=1` to seed the client-side `costTrackerCache`; cost tracker edit pages should hydrate from that cache first and then revalidate in the background to avoid a second visible load for already-opened trackers.
- Admin trip editing now uses a client-side `travelDataCache` plus trip-list hover/click prefetch; `useTripEditor` should hydrate travel data from cache first, reuse `costTrackerCache` when available for linked-expense UI, and then revalidate in the background instead of cold-loading an already-opened trip.
- `YnabImportForm` transaction rows should pass `loadExistingLink={false}` to `TravelItemSelector`; those rows represent not-yet-imported YNAB transactions, so prefetching `/api/travel-data/[tripId]/expense-links` is unnecessary noise and can surface misleading link-load errors even when the final link save succeeds.
- Route list cards in the travel editor should avoid controlled native `<details>` + `onToggle` feedback loops; prefer explicit button-driven disclosure state for route expansion/collapse, especially on large route lists.
- Route editor forms should treat redundant same-value callbacks from `AccessibleDatePicker`/`AriaComboBox` as no-ops; recreating route state on unchanged date/location values can trigger render loops when child inputs emit mount-time reconciliation updates.
- Travel editor location/route deletion should prompt for confirmation on the no-linked-expenses path; when linked expenses exist, keep using the dedicated delete warning/reassignment dialog instead of stacking a second generic confirm.
- `AccommodationInput` now surfaces parsed YAML `name` values to parent editors; accommodation add/edit forms should auto-fill the top-level accommodation name only while the field is blank or still tracking the last parsed YAML name, so manual overrides are preserved.
- The deploy script can fail after SSH succeeds because `deploy/build-and-push.sh` builds the Docker image and runs a stricter production TypeScript check than ad-hoc local flows; verify the actual `bun run build` output inside the containerized build before changing the shell wrapper.
- When offline delta sync succeeds for the active trip/cost editor, client state must advance its local last-saved snapshot from the synced queue entry and only clear the unsaved indicator if the current in-memory draft still matches that synced snapshot; otherwise newer local edits get hidden or stale unsaved/error state sticks forever.
- Conflict entries in the offline delta queue should be user-resolvable from the persistent top banner: expose dismiss/discard actions that remove stored `status: 'conflict'` entries from localStorage and emit the queue-change event so the warning clears immediately after confirmation.
- Converted cash-source UI should show both sides of the linkage: converted-source cards should surface `cashTransaction.fundingSegments` as per-source funding, and original source-currency cards should also list later cash events (conversions/refunds) that deducted from that source.
- `TravelItemSelector` hydration effects must not depend on live `expenseLinks` when `loadExistingLink={false}`; otherwise fresh empty-array identities can retrigger the clear-selection branch and make the YNAB import link-type select reset immediately after user choice.
- Cash refund-to-tracking-currency UI should capture the amount received in the tracking currency and derive the exchange rate from the refunded local amount; funded refund-to-base transactions should not be counted as new on-hand cash sources in the refunded currency group, only as deductions against that group’s remaining balance.
- `CostTrackerEditor.handleExpenseAdded` must compose updates from the latest `setCostData(prev => ...)` state, not the captured `costData` prop; otherwise sequential cash updates in one flow (for example refund-to-base followed by exchange-loss fee) can overwrite source-balance deductions and leave stale local cash on hand.
- Derived cash conversion source events are editable in place for metadata like date/title/notes, but date changes must remain between the latest funding source date and the first dependent allocation/conversion/refund date; reuse `getCashSourceDateEditConflict` in `src/app/lib/cashTransactions.ts` when enforcing that rule.
- Cost dashboard trend cards and daily drilldown views should apply `excludedCountries` to actual trip expenses before calculating history/averages; using the raw summary history makes exclusion-aware pacing cards drift from the headline metrics.
- For admin select controls with visible helper text, keep the on-screen label stable and wire state through `aria-pressed`/stateful `aria-label` or `aria-describedby` instead of changing the visible control text, so accessibility cues stay explicit without breaking predictable names.
- Cost dashboard exclusion-aware daily averages now derive from the canonical trip-day window minus the union of excluded-country itinerary coverage days, not from summing per-country `days`; `General`/`Unassigned` can change spend totals but never shrink itinerary-day denominators.
- Period-based country day totals should use unique-union semantics rather than raw summed period lengths so overlapping country periods do not inflate dashboard or country breakdown day counts.
- Editing an existing country budget in `CountryBudgetManager` must preserve non-form fields such as `periods`; clearing `amount` should only unset the amount, not replace the whole budget object and drop visit periods.
- In `CountryBudgetManager.addBudgetItem`, when editing an existing budget and clearing amount, build the edited object from a copy with `amount` explicitly removed before conditional re-add; otherwise spread-merging the existing budget preserves the stale amount even when the input is empty.
- In `EmbeddableMap`, spiderfy connector legs live in a dedicated Leaflet layer; when the Locations overlay is toggled off via layer control, also remove/re-add that spiderfy legs layer so gray connector traces never remain visible without pins.
- With `eslint-config-next@16`, consume Next's exported flat configs directly in `eslint.config.js` instead of wrapping `next/core-web-vitals` through `FlatCompat`; React Compiler diagnostics from `eslint-plugin-react-hooks@7` may need explicit severity choices, and the stack expects `zod` to expose `zod/v4` (use `zod@3.25+` or 4.x).
- Public `/api/travel-data` responses must treat top-level accommodations as private by default: only include `isAccommodationPublic` records after `serverPrivacyUtils` strips privacy flags/cost links, and never expose private accommodation IDs through public locations.
- YNAB split subtransactions should keep per-split import IDs (derived from parent import ID + subtransaction identity) and duplicate parent transaction responses from category endpoints should merge subtransaction arrays before flattening, otherwise split siblings can be skipped as duplicates.
- Host-sensitive map/calendar pages (`/map/[id]`, `/calendars/[tripId]`) must not use shared public CDN cache headers because admin hosts render planning/shadow data; keep these routes `Cache-Control: no-store` unless cache keys explicitly partition by host/admin mode.
- Route editor saves should normalize manual `routePoints` to the final saved endpoints, so public map rendering cannot silently prefer stale manual geometry after endpoint edits; valid endpoint-matched manual geometry should remain unchanged.
- OSRM routing remains enabled by default via `https://router.project-osrm.org`; operators can set `OSRM_BASE_URL` for a private backend, and `/api/routing/osrm` responses should stay `private, no-store` because route payloads can contain admin itinerary coordinates.
- Local sync scripts such as `sync-main.sh`, `sync-github.sh`, and `sync-from-github.sh` are ignored tooling; GitHub-to-Forgejo sync should replay GitHub-only commits onto local `main` with `git cherry-pick -S` and push the signed local `main`, never direct-push `github/main:main`, because direct ref pushes preserve GitHub-created commit objects and signatures.
- Root production TypeScript config now targets `es2022` under Next.js 16/SWC and no longer needs `ignoreDeprecations: "5.0"`; broad test typechecking should be staged separately because including all Jest files currently exposes existing fixture/type drift instead of a clean CI-ready check.
