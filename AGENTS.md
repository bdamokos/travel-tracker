...
- Offline queue sync runs in multiple controlled tabs at once (admin shell, trip editor, cost editor); serialize `syncOfflineDeltaQueue` across tabs with a shared localStorage lock or stale queue snapshots can be written back as false `conflict` entries after reconnect.
- Offline reliability fixes depend on three coordinated rules: app-shell caching must allow same-origin Next route HTML even when those documents are `Cache-Control: no-store`, first offline warmup must also run after the first `controllerchange`, and travel offline conflict detection must compare canonically normalized travel payloads so omitted empty editor-default fields do not become false conflicts.
- Safari/iOS offline startup needs warmed route-dependent Next assets as well as warmed HTML/data: `ServiceWorkerRegistration` warmup should parse cached route HTML for same-origin `/_next/static/*` scripts/styles and fetch them online so the service worker static cache contains the chunks Safari may not retain in its normal HTTP cache.
- On Cloudflare Access-protected deployments, protected `/_next/static/*` responses can carry `Cache-Control: no-store`; service-worker runtime caching must still allow same-origin Next static assets to enter `STATIC_CACHE`, or Safari/iOS can boot cached HTML but fail offline on uncached chunks while Chrome survives on its normal HTTP cache.
