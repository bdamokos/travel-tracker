...
- Admin/private OSRM routing must be explicit opt-in via `OSRM_BASE_URL`; when unset, `/api/routing/osrm` should return `503`/`no-store` and route generation should fall back to straight lines instead of sending coordinates to public OSRM.
