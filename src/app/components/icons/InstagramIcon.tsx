...
interface InstagramIconMarkupOptions {
  containerClassName?: string;
  iconClassName?: string;
  className?: string;
  ariaLabel?: string;
...
export const getInstagramIconMarkup = ({
  containerClassName = 'w-8 h-8',
  iconClassName = 'w-4 h-4',
  className,
  ariaLabel,
}: InstagramIconMarkupOptions = {}) => {
  ...
  const svgClassName = `text-white ${iconClassName}`.trim();
  const spanStart = `<span class="${combinedContainerClassName}"${roleAttribute}${accessibilityAttribute}>`;
  const svgMarkup = `<svg class="${svgClassName}" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" focusable="false"><path d="${SVG_PATH_D}" /></svg>`;
